...
{
    // Convert the ADC value into voltage using the formula V = (Vref / 4096) * adc_val
    // where Vref is the reference voltage of the ADC (3.3V in this case)
    float voltage = (60.0 / 4096) * adc_val;
    
    // Convert the voltage reading into temperature using the formula provided in the HL-01 datasheet
...
int main ()
{
	...
	ADC_Init ();
	ADC_Enable ();	
	UART1_SendString ("Welcome to Embedded Systems LAB\n");
	while (1)
	{
		...
		ADC_Start (1);
		ADC_WaitForConv ();
		ADC_VAL = convert_to_temperature(ADC_GetVal());
		sprintf(buffer, "ADC_Channel_4 = %f\r\n", ADC_VAL);
		...
		//sprintf(buffer, "ADC_Channel_4 = %d\r\n", ADC_VAL[1]);
		//UART1_SendString(buffer);
		//delay(10000);
